...
class Models:
    ...
    def _validate(self, raw_data: dict):
        ...
        if missing or extra:
            raise ValueError(
                f"Invalid fields - missing: {missing}, unexpected: {extra}"
            )
    ...
    def _aggregate_shap(self, shap_array: np.ndarray, ohe_feature_names: list) -> dict:
        ...
        for original_feat, ohe_cols in FEATURE_GROUPS.items():
            indices = [
                ohe_feature_names.index(c) for c in ohe_cols if c in ohe_feature_names
            ]
    ...
    def recommend(self, raw_data: dict) -> dict:
        ...
        yields = {
            crop: float(
                getattr(self, crop).predict(xgb.DMatrix(pd.DataFrame([data])))[0]
            )
